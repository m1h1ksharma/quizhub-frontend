...
import {
  FaChartBar, FaUpload, FaTrophy, FaSignOutAlt, FaTasks, FaCog,
  FaDotCircle, FaUsers, FaBars, FaTimes, FaRocket
} from "react-icons/fa";
...
function AdminSidebar() {
  ...
  const fetchLiveRound = async () => {
    try {
      const res = await API.get("/admin/settings/timer"); //[cite: 2]
  ...
  return (
    <>
      ...
      {/* Mobile Overlay[cite: 2] */}
      {isOpen && isMobile && <div onClick={toggleMenu} style={styles.overlay} />}

      <style>{`
        @keyframes blink { 0% { opacity: 1; } 50% { opacity: 0.4; } 100% { opacity: 1; } }
        .blink { animation: blink 1.5s infinite; }
        .sidebar-link { transition: 0.3s cubic-bezier(0.4, 0, 0.2, 1); color: #64748b !important; text-decoration: none; }
        .sidebar-link:hover { background: #f8fafc !important; color: #0f172a !important; transform: translateX(5px); }
        .active-sidebar-link { background: #eff6ff !important; color: #2563eb !important; font-weight: 700 !important; box-shadow: inset 4px 0 0 #2563eb; text-decoration: none; }
...
const styles = {
  ...
  logo: { fontSize: "28px", fontWeight: "900", margin: 0, letterSpacing: '-1.5px', color: "#0f172a" },
