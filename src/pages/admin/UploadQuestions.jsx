...
import {
  FaClock, FaLayerGroup, FaPlus, FaCloudUploadAlt, FaBrain, FaTimes, FaFileExcel, FaSave, FaRobot, FaDownload, FaInfoCircle
} from "react-icons/fa";
...
function UploadQuestions() {
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        Swal.fire({ title: 'Format Error', text: 'Invalid Excel structure.', icon: 'error', scrollbarPadding: false });
      }
    };
  ...
  const handleAISave = async () => {
    const target = getTargetCategory();
    if (!target || aiPreview.length === 0) return showToast('No data', 'warning');
    try {
      setLoading(true);
            ...
            <div style={{animation: 'fadeIn 0.4s ease'}}>
              ...
              {excelPreview.length > 0 && (
                <div style={styles.previewBox}>
                  <div style={styles.previewHeader}><FaFileExcel color="#10b981"/> Staged Records: {excelPreview.length}</div>
          ...
          {activeTab === "ai" && (
            <div style={{animation: 'fadeIn 0.4s ease'}}>
              <div style={styles.aiHeader}>
                ...
                <h3 style={{margin: 0, fontWeight: '900'}}>AI Generation Core</h3>
              </div>
...
const styles = {
  ...
  configCard: { background: "rgba(255,255,255,.85)", backdropFilter: 'blur(20px)', borderRadius: "28px", padding: "24px", border: "1px solid #f1f5f9" },
  ...
  input: { width: "100%", padding: "14px 18px", borderRadius: "14px", border: "1px solid #e2e8f0", outline: "none", fontSize: "14px", fontWeight: '600', color: '#1e293b' },
