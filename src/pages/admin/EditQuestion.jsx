...
function EditQuestion() {
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      try {
        // FIX: Added leading slash and backticks for dynamic ID[cite: 4]
  ...
  const showToast = (msg, icon = 'success') => {
    const Toast = Swal.mixin({
      ...
      customClass: { popup: 'swal-cool-toast' }
    });
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      // FIX: Added leading slash and backticks for update path
  ...
  if (loading) return <div style={styles.loader}>Syncing Question Data...</div>;
  ...
  return (
    <div style={{ ...styles.pageContainer, padding: isMobile ? "15px" : "40px" }}>
      ...
      <style>{`
        .save-btn-hover { transition: 0.3s; }
        .save-btn-hover:hover { transform: translateY(-3px); box-shadow: 0 10px 25px 5px rgba(37, 99, 235, 0.4); cursor: pointer; }
        .swal-cool-toast {
...
const styles = {
  ...
  saveBtn: { background: "#2563eb", color: "#fff", border: "none", padding: "18px", borderRadius: "18px", fontWeight: "900", fontSize: "16px", display: "flex", alignItems: "center", justifyContent: "center", gap: "12px" }
};
