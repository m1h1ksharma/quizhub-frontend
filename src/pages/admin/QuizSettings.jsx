...
import {
  FaClock, FaLayerGroup, FaTrash, FaTimes, FaListOl, FaPlus, FaChevronRight, FaShieldAlt, FaRocket, FaBolt
} from "react-icons/fa";
...
function QuizSettings() {
  ...
  const handleDeleteRound = async (roundName) => {
    if (roundName === settings.activeRound) {
      return Swal.fire({ ...premiumAlert, title: 'Action Restricted', text: 'You cannot delete a round that is currently LIVE.', icon: 'error' });
    }
  ...
  const handleGoLive = async () => {
    try {
      ...
      Swal.fire({
        ...premiumAlert,
        title: 'System Live!', // 🔥 "Mission Active" replaced
...
const styles = {
  ...
  roundCard: { borderRadius: "35px", padding: "30px", boxShadow: "0 10px 25px rgba(0,0,0,0.02)" },
