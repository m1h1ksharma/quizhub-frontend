  ...
  return (
    <div className="main-viewport">
      ...
      <ul className="bubbles-container">
        ...
        <li className="bubble b5"></li><li className="bubble b6"></li>
        <li className="bubble b7"></li><li className="bubble b8"></li>
      </ul>

      <div className="login-card" style={{ ...styles.card, maxWidth: "460px", padding: isMobile ? "30px 20px" : "50px" }}>
        <div style={styles.headerBox}>
          <h1 style={{...styles.title, fontSize: isMobile ? '30px' : '40px'}}><span style={{color: '#2563eb'}}>PIET</span> QUIZHUB</h1>
        ...
        <form onSubmit={handleSubmit} style={styles.formBase}>
          ...
          <div style={{marginTop: '20px', display: 'flex', gap: '10px'}}>
            ...
            <button type="submit" disabled={loading} className="btn-glow" style={styles.btn}>{loading ? "..." : (mode === "login" ? "PROCEED" : (step === 1 ? "NEXT" : "JOIN"))}</button>
          </div>
        </form>

        <div style={styles.footer}>
          <p style={{margin: 0, fontWeight: '600', color: '#94a3b8'}}>{mode === "login" ? "New User?" : "Member?"} <span style={styles.link} onClick={() => {setMode(mode === "login" ? "register" : "login"); resetForm();}}>{mode === "login" ? " Register" : " Login"}</span></p>
        </div>
      </div>
